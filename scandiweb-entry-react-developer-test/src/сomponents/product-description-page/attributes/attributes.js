...
export default class Attributes extends Component {
    ...
    render() {
        ...
        const attributesItems = attributes.length ? attributes.map(({ id, items, name, type }) => {
            return (
                <div key={id}
                     className='attribute'>
                    <div className='attr-name'>{name.toUpperCase()}:</div>
                    <div className='items'>
                        {items.map(({ id, value, displayValue }) => {

                            const clazz = type === 'text' ? 'text-attr-btn' : 'swatch-attr-btn';
                            ...
                            if (idx >= 0) {
                                clazzSelected = type === 'text' ? 'text-attr-btn-selected' : 'swatch-attr-btn-selected'}

                            return (
                                <button type='button'
                                        className={`${clazz} ${clazzSelected}`}
                                        style={{background: backgroundColor}}
                                        onClick={() => selectAttribute({type, name, id,
                                            value, displayValue})}
                                        key={id}>
